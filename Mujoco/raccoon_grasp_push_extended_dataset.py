...
class SyncSimRaccoonDataset:
    ...
    def make_push_plan(self, box_x, box_y):
        z_above = 0.08
        z_push = 0.035
        y_start = max(0.14, box_y - 0.025)
        y_end = min(0.245, box_y + 0.020)
        return [
            [box_x, y_start, z_above, 0],
            [box_x, y_start, z_push, 0],
            [box_x, y_end, z_push, 0],
            [box_x, y_end, z_above, 0],
        ]
...
def run_episode_and_record(
    ...
    try:
        ...
        if task_type == "push":
            ...
            moved_dist = float(np.linalg.norm(final_pose[:2] - np.array([target_x, target_y])))
            success = True
            logger.meta["push_moved_dist"] = moved_dist
            logger.meta["push_success_note"] = "Saved as a push demonstration; moved distance is logged for analysis."
        else:
...
def collect_dataset(
    ...
    try:
        while sum(success_counts.values()) < num_episodes and attempt_count < max_attempts:
            ...
            task_type = str(rng.choice(["grasp", "push"]))

            if task_type == "push":
                template = str(rng.choice([
...
if __name__ == "__main__":
    collect_dataset(
        xml_path="Raccoon_colored_cylinder.xml",
        dataset_root="raccoon_grasp_push_extended",
        num_episodes=20,
        colors=("red", "blue", "green", "yellow"),
        instruction_template="mixed grasp and push tasks",
        keep_failed=False,
        use_viewer=False,
        camera_name="front_view",
