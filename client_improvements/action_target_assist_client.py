...
def main():
    ...
    parser.add_argument("--instruction_template", default=DEFAULT_INSTRUCTION_TEMPLATE)
    parser.add_argument("--unnorm_key", default="raccoon_pick_place")
    parser.add_argument("--output_dir", default="rollout_outputs_target_assist")
    parser.add_argument("--episode_id", type=int, default=1)
    parser.add_argument("--max_steps", type=int, default=45)
    parser.add_argument("--max_delta_xyz", type=float, default=0.012)
    ...
    log_path = Path(args.action_log_csv or out_dir / "target_assist_log.csv")
    ...
    try:
        if args.use_real_robot:
            if RealRaccoonController is None:
                raise ImportError("RealRaccoonController를 import할 수 없습니다.")
            try:
        ...
        meta = {
            ...
            "mode": "openvla_request_with_target_assisted_action_mapping",
            "args": vars(args),
        }
        ...
        print(
            f"[TARGET_ASSIST] instruction={instruction!r} | target_color={target_color!r} | "
            f"target_xy=({target_xy[0]:.3f}, {target_xy[1]:.3f}) | log={log_path}"
        )
        ...
        with open(log_path, "w", newline="", encoding="utf-8") as f:
            ...
            for step_idx in range(args.max_steps):
                ...
                action, stage, goal_xyz, ee_pose, dist_xy = assisted_action(
                    env=env,
                    target_xy=target_xy,
                    stage=stage,
                ...
                writer.writerow(
                    {
                        ...
                        "raw_action7": json.dumps(raw_action),
                ...
                print(
                    f"[{step_idx:03d}] stage={stage} | dist_xy={dist_xy:.4f} | "
                    f"gripper={action[6]:.1f} | lift={target_lift:.4f}"
                )
